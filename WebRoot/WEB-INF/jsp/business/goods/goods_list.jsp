...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="report/goodsListPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class='center'>评优类型</th>
						<th class='center'>题目标题名</th>
					    <th class='center'>题目已投次数</th>
						<th class='center'>投票开始时间</th>
						<th class='center'>投票结束时间</th>
						<th class='center'>可以供投票选项个数</th>
						<th class='center'>题目图像</th>
						<th class='center'>题目简介</th>
						<!-- <th class='center'>选项名称</th>
						<th class='center'>本项已投次数</th>
						<th class='center'>选项类型</th>
						<th class='center'>选项图像</th>
						<th class='center'>选项简介</th> -->
						...
						<c:forEach items="${goodsList}" var="rlist" varStatus="vs">
							...
							<%-- 	<td class='center' style="width: 30px;">
								...
								<td class='center' style="width: 20px;">
									<label>
										<input type='checkbox' name='ids' value="${rlist.ATID }"/>
										<span class="lbl"></span>
									</label>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td class='center'>${rlist.AVTName}</td>
								<td class='center'>${rlist.ATName}</td>
								<td class='center'>
									<c:if test="${rlist.ATVotNum == null }">0</c:if>
									<c:if test="${rlist.ATVotNum != null }">${rlist.ATVotNum}</c:if>
								</td>
								<td class='center'>
									<fmt:formatDate type="date" value="${rlist.ATBegTime }" />
								<td class='center'>
									<fmt:formatDate type="date" value="${rlist.ATEndTime }" />
            					</td>
								<td class='center'>
									<c:if test="${rlist.ATOptionNum == null}">0</c:if>
									<c:if test="${rlist.ATOptionNum != null}">${rlist.ATOptionNum }</c:if>
								</td>
								<td class='center'><img style="width: 40px;height: 40px" alt="" src="${rlist.ATPhoto }"></td>
								<td>
									<a id="span_content" onclick="jianjie('${rlist.ATID }')" style="" onmousemove="showdiv('span_div','',event)" onmouseover="showdiv('span_div','',event)" onmouseout="showdiv('span_div','none',event)">
										<c:if test="${rlist.ATIntroduction !=null}">
											${fn:substring(rlist.ATIntroduction eq null?'无':'查看题目简介',0,6)}...
										</c:if>
									</a>
								</td>
								...
								<td>
									<a id="span_content" onclick="jianjie('${rlist.ATOID }')" style="" onmousemove="showdiv('span_div','',event)" onmouseover="showdiv('span_div','',event)" onmouseout="showdiv('span_div','none',event)">
										<c:if test="${rlist.ATOIntroduction !=null}">
											${fn:substring(rlist.ATOIntroduction eq null?'无':rlist.ATOIntroduction,0,5)}...
										</c:if>
									</a>
								</td> --%>
								<!-- 操作部分 ben-->
								<td style="width: 60px;">
									...
									<a class='btn btn-mini btn-success' title="评优选项" href="appraisingTopicsOption/AppraisingTopicsOptionListPage.do?ATID=${rlist.ATID}" onclick="appraisingTopicsOptionadds1();">选项管理</a>
									<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${rlist.ATID }')" ><i class='icon-edit'></i></a>
									<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.ATID }',true)"><i class='icon-trash'></i></a>
									</div>
								</td>
								<!-- 操作部分end -->
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//评优选项管理
		function appraisingTopicsOptionadds(){
				window.location.href='appraisingTopicsOption/AppraisingTopicsOptionListPage.do';
		}
		function appraisingTopicsOptionadd(tagID){
			var ItemIDs = tagID;
			if(ItemIDs==""){
				...
				top.jzts();
				 ...
				 diag.Drag=true;
				 diag.Title ="新增";
				 diag.URL = '<%=basePath%>appraisingTopicsOption/goAddPage.do';
				 diag.Width = 600;
				 diag.Height = 550;
				 diag.CancelEvent = function(){ //关闭事件
		...
		function add(){
			...
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>report/goAddPage.do';
			 diag.Width = 900;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function edit(tagID){
			...
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>report/edit.do?tagID='+tagID;
			 diag.Width = 900;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//查看简介
		function jianjie(ATID){
			 top.jzts();
		   	 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="评优题目简介内容";
			 diag.URL = '<%=basePath%>report/kanjianjie.do?ATID='+ATID;
			 diag.Width = 900;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(tagID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>report/del.do?ids='+tagID;
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							top.jzts();
							var url = "<%=basePath%>report/del.do?ids="+str;
							$.get(url,function(data){
								nextPage("${page.currentPage}");
							});
							$.ajax({
								type: "POST",
								url: '<%=basePath%>report/del.do',
						    	data: {ids:str},
								dataType:'json',
