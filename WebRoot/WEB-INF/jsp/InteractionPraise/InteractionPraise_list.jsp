...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="InteractionManage/InteractionPraiseListPage.do" method="post" name="residenceForm" id="residenceForm">
			<table>
				<tr>
					<td>
			...
			<table style="margin-top: 8px;" id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">点赞党员</th>
						<th class="center">被评主题</th>
						<th class="center">点赞时间</th>
		...
		<tbody>
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty InteractionPraiseList}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${InteractionPraiseList}" var="rlist" varStatus="vs">
									
							<tr>
								<td class='center' style="width: 30px;">
									<label>
										<input type='checkbox' name='ids' value="${rlist.IPID }"/>
										<span class="lbl"></span>
									</label>
								</td>
								...
								<td class='center' style="width: 10%;">
									<fmt:formatDate type="date" value="${rlist.IPTime}" />
									<%-- <fmt:formatDate type="both" dateStyle="long" timeStyle="long" value="${rlist.IPTime}" /> --%>
            					</td>
								<td class='center' style="width: 8%;">
									<%-- <a class='btn btn-mini btn-info' title="编辑" onclick="edits('${rlist.IPID }')" ><i class='icon-edit'></i></a> --%>
									<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${rlist.IPID }','${rlist.InteractionID }',true)"><i class='icon-trash'></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//删除 api/kehu/${msg }.do?shouhuo_address_id=${pd.shouhuo_address_id}&tag=${tag }&user_shangjia_id=${user_shangjia_id}
		...
		function del(tagID,InteractionID,msg){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>InteractionManage/deleteInteractionPraise.do?ids='+tagID+'&InteractionID='+InteractionID;
					$.get(url,function(data){
				//		nextPage("${page.currentPage}");
						window.location.href='InteractionManage/InteractionPraiseListPage.do?InteractionID='+InteractionID;
					});
				}
			});
		}
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							top.jzts();
							var url = "<%=basePath%>InteractionManage/deleteInteractionPraise.do?ids="+str;
							$.get(url,function(data){
								nextPage("${page.currentPage}");
							});
							$.ajax({
								type: "POST",
								url: '<%=basePath%>InteractionManage/deleteInteractionPraise.do',
						    	data: {ids:str},
								dataType:'json',
