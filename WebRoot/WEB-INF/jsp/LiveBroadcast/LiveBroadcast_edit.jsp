...
<body>
	<form action="LiveBroadcastManage/${msg }.do" id="residenceForm" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="LiveBroadcastLength" id="LiveBroadcastLength" value="${pd.LiveBroadcastLength}"/>
		<input type="hidden" name="LiveBroadcastTypes" id="LiveBroadcastTypes" value="${pd.LiveBroadcastType}"/>
		<div id="zhongxin">
		<table class="table table-striped table-bordered">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${msg == 'update' }">
					     <select name="LiveBroadcastTypes" id="LiveBroadcastTypes" title="课程类型" disabled="disabled">
					     	<option value="">请选择课程类型</option>
							<option value="0" <c:if test="${pd.LiveBroadcastType == '0' }">selected</c:if> >直播</option>
							<option value="1" <c:if test="${pd.LiveBroadcastType == '1' }">selected</c:if> >录播</option>
						 </select>    
				...
				<td>
					...
					<c:if test="${pd != null && pd.LiveBroadcastVideoUrl != '' && pd.LiveBroadcastVideoUrl != null }">
						<a style="color: black;" onchange="fileType(this)" target="_blank">
							1.<i class="icon-play-circle"></i><b>文件</b>
						</a>
						<%-- <input type="button" class="btn btn-mini btn-danger" value="删除" onclick="delP('${pd.PhotoPath}','${pd.PhotoID }');" /> --%>
			...
			<c:if test="${pd.LiveBroadcastType =='0' }">
				<tr id="1">
					<td>视频开始时间：</td>
					<td>
						...
						<div class="control-group">
			                <div class="controls input-append date form_datetime" data-date="" data-date-format="yyyy-mm-dd HH:ii" data-link-field="dtp_input1">
			                    <input style="width: 160px;" size="16" type="text" value="<fmt:formatDate value="${pd.LiveBroadcastBeginTime }" type="both" pattern="yyyy-MM-dd HH:mm"/>" name="LiveBroadcastBeginTime" id="LiveBroadcastBeginTime" readonly placeholder="视频开始时间" title="视频开始时间">
				...
				<tr id="2">
					...
					<td>
					     ...
					     <div class="control-group">
			                <div class="controls input-append date form_datetime" data-date="" data-date-format="yyyy-mm-dd HH:ii" data-link-field="dtp_input1">
			                    <input style="width: 160px;" size="16" type="text" value="<fmt:formatDate value="${pd.LiveBroadcastEndTime }" type="both" pattern="yyyy-MM-dd HH:mm"/>" name="LiveBroadcastEndTime" id="LiveBroadcastEndTime" readonly placeholder="视频结束时间" title="视频结束时间">
			                    <span class="add-on"><i class="icon-remove"></i></span>
								<span class="add-on"><i class="icon-th"></i></span>
			                </div>
							<input type="hidden" id="dtp_input1" value="" /><br/>
