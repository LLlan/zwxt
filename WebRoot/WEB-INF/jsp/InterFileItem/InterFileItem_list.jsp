...
<div class="container-fluid" id="main-container">
	<div id="page-content" class="clearfix">
		<div class="row-fluid">
			<div class="row-fluid">
				<!-- 检索  -->
				<form action="InterFile/InterFileItemListPage.do?InterFileID=${pd.InterFileID }" method="post" id="Form">
					<table>
						<tr>
							<td>
					...
					<!-- 检索  -->
					<a title="返回上一页" href="InterFile/interFileListPage.do" class="btn btn-mini btn-info" onclick="" >返回上一页</a>
					<table style="margin-top: 8px;" id="table_report" class="table table-striped table-bordered table-hover">
						...
						<!-- 开始循环 -->	
						<c:choose>
							<c:when test="${not empty InterFileItemList}">
							<c:if test="${QX.cha == 1 }">
								<c:forEach items="${InterFileItemList}" var="va" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label>
												<input type='checkbox' name='ids' value="${va.ItemID }"/>
												<span class="lbl"></span>
											</label>
										</td>
										...
										<td style="width: 60px;">
											<div class='hidden-phone visible-desktop btn-group'>
												<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${va.ItemID }','${pd.InterFileID }',true);"><i class='icon-edit'></i></a>
												<a class='btn btn-mini btn-danger' title="删除" onclick="del('${va.ItemID }','${pd.InterFileID }',true);"><i class='icon-trash'></i></a>
											</div>
										</td>
										<!-- 操作部分end -->
									</tr>
					...
					<div class="page-header position-relative">
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
								<c:if test="${QX.add == 1 }">
									<a class="btn btn-small btn-success" onclick="add('${pd.InterFileID}');">新增</a>
									<a title="批量删除" class="btn btn-small btn-danger" onclick="makeAll('确定要删除选中的数据吗?','${pd.InterFileID }',true);" ><i class='icon-trash'></i>批量删除</a>
								</c:if>
									<!--  
									<c:if test="${QX.add == 1 }">
...
<script type="text/javascript">
	...
	//新增
	function add(InterFileID){
		//window.location.href='InterFile/toInterFileItemAdd.do';
		top.jzts();
		 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>InterFile/toInterFileItemAdd.do?InterFileID='+InterFileID;
		 diag.Width = 600;
		 diag.Height = 300;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				 if('${page.currentPage}' == '0'){
					 ...
					 window.location.href='InterFile/InterFileItemListPage.do?InterFileID='+InterFileID;
				 }
			}
			diag.close();
		 };
	...
	//修改
	function edit(id,InterFileID){
		 //window.location.href='InterFile/toEditItem.do?ItemID='+id;
		top.jzts();
	   	 var diag = new top.Dialog();
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>InterFile/toEditItem.do?ItemID='+id;
		 diag.Width = 600;
		 diag.Height = 300;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
	function del(id,InterFileID){
		bootbox.confirm("确定要执行删除操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = "<%=basePath%>InterFile/deleteFileItem.do?ids="+id;
	...
	//批量操作
	function makeAll(msg,InterFileID){
		bootbox.confirm(msg, function(result) {
			if(result) {
				...
				if(str==''){
					...
					if(msg == '确定要删除选中的数据吗?'){
						top.jzts();
						var url = "<%=basePath%>InterFile/deleteFileItem.do?ids="+str;
						$.get(url,function(data){
							//nextPage("${page.currentPage}");
							window.location.href='InterFile/InterFileItemListPage.do?InterFileID='+InterFileID;
						});
						/*$.ajax({
							type: "POST",
							url: '<%=basePath%>InterFile/deleteFileItem.do',
					    	data: {ids:str},
							dataType:'json',
