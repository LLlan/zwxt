...
import org.springframework.web.servlet.handler.HandlerInterceptorAdapter;
  ...
  public class LoginHandlerInterceptor extends HandlerInterceptorAdapter{
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
     ...
     if (path.matches(".*/((login)|(logout)|(code)|(app)|(main)|(client)|(servlet)|(uploadFiles)|(resource)|(weixin)|(group)|(api)|(static)|(plugins)|(websocket)|(XiaoQufw)|(familyInfo)).*")) {
       return true;
      }
     ...
     if (user != null){
       ...
       for (int i = 0; i < menuList.size(); ++i) {
         for (int j = 0; j < ((Menu)menuList.get(i)).getSubMenu().size(); ++j) {
        	// System.out.println("------i-----"+((Menu)menuList.get(i)).getSubMenu().get(j).getMENU_URL());
           if (((Menu)((Menu)menuList.get(i)).getSubMenu().get(j)).getMENU_URL().split(".do")[0].equals(path.split(".do")[0])) {
        	 ...
        	 //菜单地址为空或者#,跳转到登录页面
            if (((Menu)menuList.get(i)).getSubMenu().get(j).getMENU_URL().equals("#")||
             ...
             System.out.println("---------权限角色名字---------" + ROLE_NAME);
  
             String[] cha_rolename = {"系统管理员", "超级管理员", "普通管理员", "普通用户"};
             String[] add_rolename = {"系统管理员", "超级管理员"};
             String[] edit_rolename = {"系统管理员", "超级管理员"};
             String[] del_rolename = {"系统管理员"};
  
             map.put("cha", ((RightsHelper.testRights((String)map.get("chas"), MENU_ID)) || (ROLE_NAME.equals(cha_rolename[0])) || (ROLE_NAME.equals(cha_rolename[1])) || (ROLE_NAME.equals(cha_rolename[2])) || (ROLE_NAME.equals(cha_rolename[3]))) ? "1" : "0");
             ...
             map.put("del", ((RightsHelper.testRights((String)map.get("dels"), MENU_ID)) || (ROLE_NAME.equals(del_rolename[0]))) ? "1" : "0");
